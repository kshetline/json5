...
const ValueSourceWrapper = util.ValueSourceWrapper
...
module.exports = function parse (text, theReviver) {
    source = String(text)
    parseState = 'start'
    stack = []
    lastPos = 0
...
const lexStates = {
    ...
    value () {
        ...
        case 'I':
            read()
            literal('nfinity')
            return newPrimitiveToken('numeric', Infinity)

        case 'N':
            read()
            literal('aN')
            return newPrimitiveToken('numeric', NaN)
    ...
    sign () {
        ...
        case 'I':
            read()
            literal('nfinity')
            return newPrimitiveToken('numeric', sign * Infinity)
    ...
    zero () {
        ...
        return newPrimitiveToken('numeric', sign * 0)
    },
    ...
    string () {
        ...
        case '"':
            if (doubleQuote) {
                read()
                return newPrimitiveToken('string', buffer)
            }
...
function newPrimitiveToken (type, value) {
    ...
    if (typeof reviver === 'function' && reviver.length > 2 && parseState !== 'beforePropertyName') {
